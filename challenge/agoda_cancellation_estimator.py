...
class AgodaCancellationEstimator(BaseEstimator):
    ...
    def adjust_response(X):
        low_range = datetime.datetime(2018, 12, 7).date()
        high_range = datetime.datetime(2018, 12, 13).date()
        y = np.zeros(X.shape[0])
        for sample in range(X.shape[0]):
            if X[sample].dt.date>=low_range and X[sample].dt.date <= high_range:
                y[sample] = 1
            else:
                y[sample] = 0
        return y
        # y = np.zeros(X.shape[0])
        # for sample in range(X.shape[0]):
        #     if isinstance(X[sample], numpy.datetime64):
        #         y[sample] = 1
        #     else:
        #         y[sample] = 0
        # return y
